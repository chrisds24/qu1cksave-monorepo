...
CREATE TABLE job(id UUID UNIQUE PRIMARY KEY DEFAULT gen_random_uuid(), member_id UUID NOT NULL, resume_id UUID, cover_letter_id UUID, title VARCHAR(255) NOT NULL, company_name VARCHAR(255) NOT NULL, job_description TEXT, notes TEXT, is_remote VARCHAR(10) NOT NULL, salary_min integer, salary_max integer, country VARCHAR(255), us_state VARCHAR(3), city VARCHAR(255), date_saved TIMESTAMPTZ NOT NULL DEFAULT NOW(), date_applied jsonb, date_posted jsonb, job_status VARCHAR(20) NOT NULL, links jsonb, found_from VARCHAR(255));
